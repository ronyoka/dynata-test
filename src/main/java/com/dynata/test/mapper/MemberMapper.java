...
import static com.dynata.test.mapper.CsvColumnConstants.EMAIL_ADDRESS;
import static com.dynata.test.mapper.CsvColumnConstants.EMAIL_EMPTY;
import static com.dynata.test.mapper.CsvColumnConstants.FULL_NAME;
import static com.dynata.test.mapper.CsvColumnConstants.FULL_NAME_EMPTY;
import static com.dynata.test.mapper.CsvColumnConstants.IS_ACTIVE;
import static com.dynata.test.mapper.CsvColumnConstants.IS_ACTIVE_EMPTY;
import static com.dynata.test.mapper.CsvColumnConstants.MEMBER_ID;
...
public interface MemberMapper extends BaseCsvRecordMapper {
    ...
    default int toMemberId(CSVRecord record) {
        return getCsvColumnAsInteger(record, MEMBER_ID).orElseThrow(() -> new IllegalStateException(MEMBER_ID_EMPTY));
    }

    @Named("toFullName")
    default String toFullName(CSVRecord record) {
        return getCsvColumnAsString(record, FULL_NAME).map(String::trim).orElseThrow(() -> new IllegalStateException(FULL_NAME_EMPTY));
    }

    @Named("toEmail")
    default String toEmail(CSVRecord record) {
        return getCsvColumnAsString(record, EMAIL_ADDRESS).orElseThrow(() -> new IllegalStateException(EMAIL_EMPTY));
    }

    @Named("toActive")
    default boolean toActive(CSVRecord record) {
        return getCsvColumnAsString(record, IS_ACTIVE)
