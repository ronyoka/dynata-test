...
import static com.dynata.test.mapper.CsvColumnConstants.COMPLETION_POINTS;
import static com.dynata.test.mapper.CsvColumnConstants.COMPLETION_POINTS_EMPTY;
import static com.dynata.test.mapper.CsvColumnConstants.EXPECTED_COMPLETES;
import static com.dynata.test.mapper.CsvColumnConstants.EXPECTED_COMPLETES_EMPTY;
import static com.dynata.test.mapper.CsvColumnConstants.FILTERED_POINTS;
...
import static com.dynata.test.mapper.CsvColumnConstants.SURVEY_ID;
...
public interface SurveyMapper extends BaseCsvRecordMapper {
    ...
    default int toSurveyId(CSVRecord record) {
        return getCsvColumnAsInteger(record, SURVEY_ID).orElseThrow(() -> new IllegalStateException(SURVEY_ID_EMPTY));
    }

    @Named("toName")
    default String toName(CSVRecord record) {
        return getCsvColumnAsString(record, NAME).orElseThrow(() -> new IllegalStateException(SURVEY_NAME_EMPTY));
    }

    @Named("toExpectedCompletes")
    default int toExpectedCompletes(CSVRecord record) {
        return getCsvColumnAsInteger(record, EXPECTED_COMPLETES).orElseThrow(() -> new IllegalStateException(EXPECTED_COMPLETES_EMPTY));
    }

    @Named("toCompletionPoints")
    default int toCompletionPoints(CSVRecord record) {
        return getCsvColumnAsInteger(record, COMPLETION_POINTS).orElseThrow(() -> new IllegalStateException(COMPLETION_POINTS_EMPTY));
    }

    @Named("toFilteredPoints")
    default int toFilteredPoints(CSVRecord record) {
        return getCsvColumnAsInteger(record, FILTERED_POINTS).orElseThrow(() -> new IllegalStateException(FILTERED_POINTS_EMPTY));
    }
}
